...
probs = pd.read_csv("Gamma_probs_0-100_tax-filtered_head-filtered.csv", index_col="eggNOG")
LL_ratios = pd.Series()
...
print "Total LexA genes:", sum(all_genes.eggNOG == lexa_cog)
...
print "LexA genes (head complete + Gamma):", sum(genes.eggNOG == lexa_cog)

if sum(genes.eggNOG == lexa_cog) == 0:
    sys.exit("No LexA COGs to analyze.")
...
print
print "PSSM:", pssm
print "LexA scores: mean = %.2f, std = %.2f, max = %.2f" % (np.mean(lexa_scores_flat), np.std(lexa_scores_flat), np.max(lexa_scores_flat))
print
